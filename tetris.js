...
function SetupCanvas(){
    ...
    tertisLogo = new Image(130, 65);
    tertisLogo.onload = DrawTetrisLogo;
    tertisLogo.src = 'tetris.png';
...
function HandleKeyPress(key){
    if(winOrLose != 'Game Over'){
        if(key.keyCode === 65){
            ...
            if(!HittingTheWall() && !CheckForHorizontallCollision()){
                DeleteTetromino();
                startX--;
                DrawTetromino();
            }
...
function CheckForHorizontallCollision(){
    let tetrominoCopy = curTetromino;
    let collision = false;
    for(let i = 0; i < tetrominoCopy.length; i++){
        let square = tetrominoCopy[i];
